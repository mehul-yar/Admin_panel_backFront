...
const CreateProductForm = () => {
  ...
  const [productData, setProductData] = useState({
    ...
    size: "",
    productDetails: "",
    images: [], // Array to store multiple image files
  });
  ...
  const handleSubmit = async (e) => {
    ...
    productData.images.forEach((image, index) => {
      formData.append(`images`, image); // Backend should accept field `images[]` as array
    });

    try {
      // Send a POST request to create the product with the images
      const response = await axios.post("http://localhost:3000/api/auth/product", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
